...
def calculate_obj(X, y, w, lambda1, lambda2, T):
    return 1 / 2 * (np.linalg.norm(y - np.dot(X, w), 'fro')) ** 2 + lambda1 * np.abs(w).sum() + lambda2 * np.abs(
        np.dot(T, w)).sum()


def graph_fs(X, y, edge_list=None, lambda1=0.8, lambda2=0.8, rho=5, max_iter=300, verbose=False, **kwargs):
    """
    ...
    if not edge_list:
        ValueError('Error using function, the network structure E/edge_list is required')
